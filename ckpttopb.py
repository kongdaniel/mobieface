...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        frozen_graphdef = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def, ['embeddings'])
        #converter = tf.lite.TFLiteConverter.from_session(sess, [inputs], [embeddings])
        converter = tf.lite.TFLiteConverter(frozen_graphdef, [inputs], [embeddings])
        #tflite_model = converter.convert()
        open("./converted_model.tflite", "wb").write(converter)
